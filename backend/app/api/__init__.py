...
async def predict(payload: PaymentFeatures, model: RecoveryModel = Depends(get_model)) -> PredictResponse:
    """
    Given payment + customer features, return the ML recovery probability.

    The probability comes from the trained GradientBoostingClassifier — NOT
    from a hardcoded value. The model file must exist (run train_model.py first).
    """
    # Build dicts matching what build_inference_row expects
    payment_data = {
...
async def model_info(
    ...
    """
    Returns the actual training metadata for the active model,
    fetched from the ml_model_metadata database table.
    """
    result = await db.execute(
        text("""
            SELECT model_version, algorithm, roc_auc, precision, recall, accuracy,
                   training_samples, test_samples, feature_importances_json,
                   trained_at
    ...
    row = result.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="No trained model found. Run train_model.py first.")

    import json
    fi_raw = json.loads(row.feature_importances_json or "[]")
    fi_list = [FeatureImportance(**fi) for fi in fi_raw[:15]]

    summary = model.model_summary()
    positive_rate = round(row.training_samples / (row.training_samples + row.test_samples), 4)
